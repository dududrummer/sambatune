...
export function ProgressionAudio({ measures, voicings, onMeasureChange, mode = 'progression' }: Props) {
  ...
  const handlePlay = useCallback(async () => {
    ...
    await startPlayback(measures, style, bpm, metronome, audioMode, vm, idx => onMeasureChange(idx), loop);
    setIsPlaying(true);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <ToggleGroup type="single" value={audioMode} onValueChange={v => v && setAudioMode(v as AudioMode)} className="flex flex-wrap gap-2">
          {MODES.map(m => (
      ...
      {isPlaying && (
        <div className="text-center space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">{MODES.find(m => m.value === audioMode)?.label}</p>
        </div>
      )}
    </div>
  );
